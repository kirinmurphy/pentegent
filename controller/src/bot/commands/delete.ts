      ...
      if (detected.type === IDENTIFIER_TYPE.JOB_ID) {
        ...
        if (detected.type === IDENTIFIER_TYPE.URL) {
          try {
            ...
            await ctx.reply(`Invalid URL: ${detected.value}`);
            return;
          }
        }
...
export async function handleConfirm(
  ...
  try {
    ...
    const target =
      pending.type === DELETION_TYPE.ALL
        ? "all scans"
        : pending.type === DELETION_TYPE.JOB
          ? "scan"
          : `scan${result.deleted > 1 ? "s" : ""} for ${pending.identifier}`;

    await ctx.reply(`Deleted ${result.deleted} ${target}`);
  } catch (err) {
    confirmationManager.clearPending(chatId);
    await ctx.reply(
