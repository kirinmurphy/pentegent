...
create_test_target() {
  ...
  response=$(curl -sf -X POST "$SCANNER_URL/targets" \
...
create_scan_job() {
  ...
  response=$(curl -sf -X POST "$SCANNER_URL/scan" \
    -H "Content-Type: application/json" \
    -d "{
...
wait_for_job_completion() {
  ...
  while [ $attempt -le $max_attempts ]; do
    response=$(curl -sf "$SCANNER_URL/jobs/$job_id")
...
list_jobs() {
  ...
  response=$(curl -sf "$SCANNER_URL/jobs?limit=5&offset=0")
  ...
  echo "$response" | jq '.jobs[] | {jobId, targetId, scanType, status, createdAt}'
}
...
run_full_verification() {
  ...
  curl -sf "$SCANNER_URL/targets" | jq '.targets[] | {id, base_url}' || log_warning "Failed to list targets"
  echo ""

  # Step 3: Create headers scan
  log_info "=== Testing Headers Scan ==="
  job_id=$(create_scan_job "headers")
  if [ $? -ne 0 ]; then
    return 1
  fi
  echo ""
  ...
  # Step 5: Check report files
  check_report_files "$job_id" || return 1
  echo ""

  # Step 6: Test crawl scan
  log_info "=== Testing Crawl Scan ==="
  job_id=$(create_scan_job "crawl")
  if [ $? -ne 0 ]; then
    return 1
  fi
  echo ""
  ...
  # Step 7: Test combined scan
  log_info "=== Testing Combined Scan (all) ==="
  job_id=$(create_scan_job "all")
  if [ $? -ne 0 ]; then
    return 1
  fi
  echo ""
